...
export default function StandingsTable({ teams, isCompact = false }: StandingsTableProps) {
        ...
        <tbody className="divide-y divide-slate-800/40">
          {sortedTeams.map((team, idx) => (
            <motion.tr 
              ...
              className={`group transition-colors ${idx < 4 ? 'bg-slate-800/5' : ''} hover:bg-slate-800/20`}
            >
              ...
              <td className="px-4 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className={`text-sm font-bold ${idx < 4 ? 'text-white' : 'text-slate-300'}`}>{team.name}</p>
